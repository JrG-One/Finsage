  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (geminiData.amount) {
        ...
        else if (lowerText.includes("interest") || lowerText.includes("dividend")) setSource("Investments Return");
        else setSource("Other");

        setDate(new Date());
      } else {
        toast.error("❌ Failed to extract income amount.");
      }
    } catch (err) {
      console.error("OCR/Gemini error:", err);
      toast.error("Error extracting from receipt");
    } finally {
      setLoading(false);
    }
  };
